...
The project is supported by three main groups of references: public physiological datasets from PhysioNet for vital-sign processing, IV drip-rate monitoring literature for optical drop detection and flow-rate estimation, and Zigbee/Edge AI documentation for deploying a low-power multi-node monitoring system on Silicon Labs hardware. These references justify the system architecture, sensing approach, wireless communication choice, and edge AI deployment strategy.# Smart IV & Patient Safety Monitoring System  
## Reference Materials Organized by Topic
